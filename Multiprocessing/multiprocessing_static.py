...
RESULTS_CSV = RESULTS_DIR / f"results_static_laptop{LAPTOP_ID}.csv"


def worker_static(image_paths):
...
def run_static(image_paths, n_processes):
    ...
    if n_processes == 1:
        worker_static(image_paths)
    else:
        chunks = [
            list(chunk)
            for chunk in np.array_split(image_paths, n_processes)
            if len(chunk) > 0
        ]

        ctx = mp.get_context("spawn")
        processes = [
            ctx.Process(target=worker_static, args=(chunk,))
            for chunk in chunks
        ]
